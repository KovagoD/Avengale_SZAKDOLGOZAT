...
public class Item
{
    ...
    public void randomizeStats()
    {
        ...
        attributes[0] = Random.Range(attributes[0], attributes[0] + _characterStats.getPercentOfHealth(_percent));
        attributes[1] = Random.Range(attributes[1], attributes[1] + _characterStats.getPercentOfResource(_percent));
        attributes[2] = Random.Range(attributes[2], attributes[2] + _characterStats.getPercentOfDamage(_percent));
        attributes[3] = Random.Range(attributes[3], attributes[3] + _characterStats.getPercentOfMoney(_percent));

        if (_characterStats.Player_level > 3)
        {
            level = Random.Range(_characterStats.Player_level - 2, _characterStats.Player_level + 2);
        }
        else
        {
            level = Random.Range(_characterStats.Player_level, _characterStats.Player_level + 2);
        }
    }
}
